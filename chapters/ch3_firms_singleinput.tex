...
\begin{proposition*}
    Given a twice-differentiable increasing and concave objective function $F(x; z)$, with choice variable $x$ and exogenous variable $z$, and $x^*(z) = \argmax_{x} F(x; z)$, then $\frac{dx^*}{dz} = - \frac{F_{xz}}{F_xx}(x^*(z); z)$. 
\end{proposition*}
...
\begin{proof}
    ...
    \begin{align*}
        \frac{dx^*}{dz} = -\frac{F_{xz}(x^*, z)}{F_xx(x^*, z)}
    \end{align*}
\end{proof}
