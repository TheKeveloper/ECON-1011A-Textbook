...
\todo{Make footnote on bar LR/SR definition}
...
\end{theorem}
\todo{Mention that we don't prove it}
...
\begin{definition}
Given points $x, y \in \R^n$, a \vocab{convex combination} of $x$ and $y$ is 
$$\lambda x + (1-\lambda)y$$
...
\todo{Reference Math Review Instead}
\begin{definition}
A function is \vocab{convex} if for all $\lambda \in (0, 1)$, we have
