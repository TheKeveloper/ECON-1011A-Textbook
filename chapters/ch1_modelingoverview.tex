...
\begin{itemize}
    \item How does the amount of workers hired depend on the minimum wage?
    \item How does the amount people choose to save depend on the interest rate?
    \item How does the amount of goods consumed depend on the tax rate? 
\end{itemize}
\todo{this could be improved}

A common point of confusion among students is the distinction between $\vec{x}$, which is the name that we give to the choice variable, and $\vec{x}^*$, which is the value that optimizes the objective function. $\vec{x}$ can in principle be any value. For example, lets say that $\vec{x} = (a, b)$, where $a$ is the number of apples you buy and $b$ is the number of bananas. You could in principle buy $a = 2$ apples and $b = 3$ bananas, even though you would be happier with $a = 4$ and $b = 4$. Importantly, it would not make sense to take comparative statics with respect to $a$ or $b$. That is, it would not make sense to ask how the number of apples you buy will depend on price if you can freely choose how many apples to buy and have not defined a relationship with price. 
